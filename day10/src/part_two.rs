...
fn get_trail_score(grid: &Grid<u32>, start: &Coordinate) -> u32 {
    ...
    for i in (0..10u32).rev() {
            ...
            .is_some_and(|coord| grid.get(coord).is_some_and(|val| *val == i))
        {
            ...
            .filter_map(|a| *a)
            {
                // If neighbor is in the grid, and has the right value, push it
                if grid.encompasses(neighbor) && grid.get(neighbor).is_some_and(|val| *val == i - 1)
                {
